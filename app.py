...
# -------------------------------------------------------------
st.set_page_config(
    page_title="통합 블로그 자동화 공장 (V2.21 모바일 단문 최적화)",
    page_icon="💰",
    layout="wide"
)

st.title("💰 네이버 & 구글 블로그 통합 수익화 자동화 머신 (V2.21)")
st.markdown("Gemini 3.6 Flash / 2500자 딥다이브 · 모바일 초간격 단문 스타일 적용")
...
# -------------------------------------------------------------
# 2. 본문 생성 버튼 (2500자 롱폼 & 모바일 초간격 단문 스타일)
# -------------------------------------------------------------
st.markdown("---")
...
if generate_btn:
    if not api_key_input:
        ...
        try:
            ...
            progress_text.text("✍️ Gemini 3.6 모델이 모바일 가독성 높은 2500자 심층 포스팅을 집필 중입니다...")
            ...
            [필수 작성 규칙 및 모바일 단문 스타일 제약]:
            1. **[분량 확보] 공백 포함 총 2,500자 이상의 충분하고 깊이 있는 롱폼(Long-form) 분량으로 작성할 것. (내용이 길어져도 아래의 단문 스타일을 끝까지 유지할 것)**
            2. **[모바일 초간격 단문 스타일 핵심]** 
               - 절대 긴 문장이나 줄글 형태로 길게 뭉쳐서 쓰지 마세요.
               - 스마트폰 화면에서 세로로 빠르게 훅훅 읽힐 수 있도록, 한 문장 또는 의미 단위의 짧은 구절마다 엔터(줄바꿈)를 자주 쳐서 시원시원한 여백을 만드세요.
               - 예시:
                 안녕하세요! 새로운 출발을 준비하며
                 소중한 보금자리를 알아보는 청년 여러분 반가워요.
               - 위 예시처럼 모든 문단과 문장을 짧게 끊어서 작성할 것.
            3. 메인 키워드('{target_keyword}')가 전체 글에서 정확히 총 **4회** 들어가도록 계산해서 배치할 것. (선택된 배치 전략 준수)
            4. '##', '###' 같은 무거운 마크다운 소제목 기호나 '**' 같은 강조 기호는 절대로 사용하지 말 것.
            ...
            progress_text.empty()
            st.success("🎉 모바일 단문 스타일이 적용된 2500자 블로그 본문이 생성되었습니다!")
