...
# -------------------------------------------------------------
st.set_page_config(
    page_title="구글 애드센스 수익화 블로그 공장 (V2.8)",
    page_icon="💰",
    layout="wide"
)

st.title("💰 구글 블로그스팟 수익화 자동화 머신 (V2.8 모바일 가독성 극대화)")
st.markdown("Gemini 3.6 Flash + Google Search Grounding / 4회 키워드 최적화 / 모바일 1~2줄 숏폼 엔터 최적화")
...
if generate_btn:
    if not api_key_input:
        ...
        try:
            ...
            # 구글 실시간 검색 툴(Google Search) 장착 옵션 설정 (Gemini 3.6 Flash 활용)
            ...
            # 프로 SEO 분석 에이전트
            progress_text.text("📊 프로 SEO 에이전트가 모바일 가독성과 키워드 4회 배치 상태를 검수 중입니다...")
            ...
            progress_text.empty()
            st.success("🎉 모바일 가독성과 팩트체크가 완벽히 적용된 블로그 패키지가 생성되었습니다!")
