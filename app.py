...
# -------------------------------------------------------------
st.set_page_config(
    page_title="통합 블로그 자동화 공장 (V2.22 인용구 마커 최적화)",
    page_icon="💰",
    layout="wide"
)

st.title("💰 네이버 & 구글 블로그 통합 수익화 자동화 머신 (V2.22)")
st.markdown("Gemini 3.6 Flash / 2500자 딥다이브 · 모바일 단문 + 인용구 태그 자동 마킹")
        ...
        if not api_key_input:
            ...
            with st.spinner("프로 SEO 에이전트가 모바일 가독성과 키워드 배치를 검수 중입니다..."):
                try:
                    ...
                    2. 모바일 초간격 단문 가독성 점수 (30점 만점): 모바일에서 읽기 편하게 짧은 호흡과 여백이 잘 적용되었는지 평가.
...
# -------------------------------------------------------------
# 2. 본문 생성 버튼 (2500자 롱폼 & 모바일 초간격 단문 스타일 + 인용구 마커)
# -------------------------------------------------------------
st.markdown("---")
generate_btn = st.button("🚀 1단계: 모바일 최적화 2500자 블로그 글 생성하기", type="primary", use_container_width=True)
...
if generate_btn:
    if not api_key_input:
        ...
        try:
            ...
            progress_text.text("✍️ Gemini 3.6 모델이 인용구 마커와 모바일 단문이 적용된 2500자 심층 포스팅을 집필 중입니다...")
            ...
            [필수 작성 규칙 및 모바일 단문 & 인용구 스타일 제약]:
            1. **[분량 확보] 공백 포함 총 2,500자 이상의 충분하고 깊이 있는 롱폼(Long-form) 분량으로 작성할 것.**
            2. **[모바일 초간격 단문 스타일 핵심]** 
               - 절대 긴 문장이나 줄글 형태로 길게 뭉쳐서 쓰지 마세요.
               - 스마트폰 화면에서 세로로 빠르게 훅훅 읽힐 수 있도록, 한 문장 또는 의미 단위의 짧은 구절마다 엔터(줄바꿈)를 자주 쳐서 시원시원한 여백을 만드세요.
            3. **[인용구 및 순서 마커 적용 규칙]**
               - 본문 중간에 핵심 포인트나 순서(첫 번째, 두 번째, 하나, 둘, 셋 등 핵심 단계나 팁)를 설명하는 구간에는 해당 문장 앞뒤로 반드시 **[인용구시작]**과 **[인용구끝]** 태그를 감싸서 출력할 것.
               - 예시: 
                 [인용구시작]
                 첫 번째로 꼭 확인하셔야 할 핵심 조건입니다.
                 [인용구끝]
               - 블로그 작성자가 에디터에서 한눈에 인용구(따옴표 박스)로 빠르게 바꿀 수 있도록 중요 포인트마다 이 태그를 적극적으로 활용할 것.
            4. 메인 키워드('{target_keyword}')가 전체 글에서 정확히 총 **4회** 들어가도록 계산해서 배치할 것. (선택된 배치 전략 준수)
            5. '##', '###' 같은 무거운 마크다운 소제목 기호나 '**' 같은 강조 기호는 절대로 사용하지 말 것. (단, [인용구시작]/[인용구끝] 태그는 예외)
            6. 100% 긍정적이고 희망찬 마인드셋을 유지할 것 (독자의 불안감을 유발하는 표현 금지).
            7. 글의 맨 마지막 줄에 아래 양식 그대로 메타데이터 및 네이버 최적화 요소를 반드시 포함해서 출력할 것:
            ...
            progress_text.empty()
            st.success("🎉 인용구 마커와 모바일 단문이 적용된 2500자 블로그 본문이 생성되었습니다!")
...
if st.session_state["generated_content"]:
    ...
    with tab_blog1:
        ...
        st.info("💡 네이버 블로그에 글을 복사해 붙여넣으신 후, [인용구시작] / [인용구끝] 태그가 있는 구간을 블로그 에디터의 '인용구' 기능으로 감싸주시면 완벽합니다!")
